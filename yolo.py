...
class YOLO(object):
    _defaults = {
        "model_path": 'model_data/yolov3_weights.pth',
    ...
    #                  获得所有的分类
    #---------------------------------------------------#
    def _get_class(self):   # expanduser把path中包含的"~"和"~user"转换成用户目录
    ...
    def generate(self):
        ...
        # 每个特征点有3个anchor，decode一次只能对一个anchor，故循环3次
        self.yolo_decodes = []
        for i in range(3):
    ...
    def detect_image(self, image):
        # np.shape(image) = 416 × 416 ×3
        image_shape = np.array(np.shape(image)[0:2])

        crop_img = np.array(letterbox_image(image, (self.model_image_size[0], self.model_image_size[1])))
        photo = np.array(crop_img, dtype=np.float32)
        photo /= 255.0 # 图片归一化
        photo = np.transpose(photo, (2, 0, 1))  # pytorch中的通道数在最前面
